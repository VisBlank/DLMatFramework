...
parser.add_argument('--port', type=int, required=False, default=5000, help='HTTP Port')
...
def init_tensorflow_model(gpu, model_path):
    ...
    # "C:/Users/ricbur01/Work/Tensorflow/MNist_1.0/check/model-ckpt-37"
...
def get_angle_from_file():
    ...
    # Convert file to an image (This way we don't need to save the image to disk)
    ...
    offsets = sess.run(model_out, feed_dict={model_in: [img_array], model_drop: 1.0})[0][0]

    return jsonify(output=float(offsets))
...
@app.route('/')
def index():
    return 'Server is running okay - Richard '
...
def get_angle_from_data():
    ...
    # Transform data to numpy array, data type should be float32 (what we trained on)
    img_array = np.array(request_data, dtype=np.float32) #.reshape([int(request_rows),int(request_cols), int(request_depth)])
    ...
    # Get steering angle from tensorflow model (Also convert from rad to degree)
    offsets = sess.run(model_out, feed_dict={model_in: [img_array], model_drop: 1.0})
    print(offsets)
    offsets /= 10
    offsets = offsets.tolist() #convert our np array to a list so we can jsonify it
    #return jsonify(output=float(offsets))
    return jsonify(output = (offsets))
